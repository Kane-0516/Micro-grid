...
function getPhaseLabel(vl: VoltageLevel, lang: string): string {
  const params = VOLTAGE_PARAMS[vl];
  return lang === 'en' ? params.phaseDescEn : params.phaseDescZh;
}

interface InverterContextBannerProps {
  lang: string;
  inverterKw: number;
  inverterCount: number;
...
function InverterContextBanner({
  lang,
  inverterKw,
  inverterCount,
  phaseLabel,
  hasVoltage,
}: InverterContextBannerProps) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: '1rem',
      padding: '0.75rem 1.1rem',
      background: 'var(--theme-tone-bg)', border: '1px solid var(--theme-tone-border)',
      borderRadius: '10px',
    }}>
      ...
      <div style={{ flex: 1, fontSize: '0.83rem', color: 'var(--theme-tone-text)', lineHeight: 1.6 }}>
        {lang === 'en'
...
interface VoltageLevelOptionProps {
  level: VoltageLevel;
  active: boolean;
  lang: string;
  inverterKw?: number;
  inverterCount?: number;
  onSelect: (level: VoltageLevel) => void;
}

function VoltageLevelOption({
  level,
  active,
  lang,
  inverterKw,
  inverterCount,
  onSelect,
}: VoltageLevelOptionProps) {
  const p = VOLTAGE_PARAMS[level];
  const showInverterSummary = !!(inverterKw && inverterCount);
  ...
  return (
    <div
      ...
      style={{
        ...
        background: active ? 'var(--theme-tone-bg)' : 'white',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <span style={{
            ...
            borderRadius: '8px',
            background: active ? 'var(--theme-brand-100)' : '#f7fafc',
        ...
        <div style={{
          ...
          {showInverterSummary && (
            <span style={{
              ...
              fontWeight: 700, color: '#1a365d',
              background: 'var(--theme-tone-bg)', borderRadius: '6px',
...
interface CurrentInputSectionProps {
  lang: string;
  voltageLevel: VoltageLevel | null;
  currentInput: string;
  estLoadKw: number | null;
  hasCurrent: boolean;
  onQuickCurrent: (value: number) => void;
  onCurrentChange: (value: string) => void;
}

function CurrentInputSection({
  lang,
  voltageLevel,
  currentInput,
  estLoadKw,
  ...
  return (
    <>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.45rem', marginBottom: '0.65rem' }}>
        {(QUICK_CURRENTS[voltageLevel] ?? []).map(a => {
          const active = Math.abs((Number.parseFloat(currentInput) || 0) - a) < 0.5;
          return (
            <button
              key={a}
              type="button"
              onClick={() => onQuickCurrent(a)}
              style={{
      ...
      {hasCurrent && estLoadKw !== null && (
        <div style={{
          ...
          <span style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--theme-brand-700)' }}>
            {estLoadKw} kW
          </span>
...
export default function StepDIYSetup({
  ...
  const currentA = Number.parseFloat(currentInput);
  const hasCurrent = !Number.isNaN(currentA) && currentA > 0;
  const estLoadKw = voltageLevel && hasCurrent ? estimateLoadKw(voltageLevel, currentA) : null;
  const phaseLabel = voltageLevel ? getPhaseLabel(voltageLevel, lang) : null;
  const showInverterBanner = !!(inverterKw && inverterCount && inverterKw > 0 && inverterCount > 0);
  ...
  const handleCurrentChange = (val: string) => {
    ...
    if (!Number.isNaN(num) && num > 0) handleCurrent(num);
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.6rem' }}>
      {showInverterBanner && (
        <InverterContextBanner
          lang={lang}
          inverterKw={inverterKw!}
          inverterCount={inverterCount!}
          phaseLabel={phaseLabel}
          hasVoltage={!!voltageLevel}
        />
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {VOLTAGE_LEVELS.map(level => (
            <VoltageLevelOption
              key={level}
              level={level}
              active={voltageLevel === level}
              lang={lang}
              inverterKw={inverterKw}
              inverterCount={inverterCount}
              onSelect={handleVoltage}
            />
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <div style={{
          fontSize: '0.81rem', color: 'var(--theme-tone-warm-text)',
          padding: '0.55rem 0.85rem', marginBottom: '0.65rem',
          background: 'var(--theme-tone-warm-bg)', borderLeft: '3px solid var(--theme-tone-warm-accent)',
        ...
        <CurrentInputSection
          ...
          onQuickCurrent={handleQuickCurrent}

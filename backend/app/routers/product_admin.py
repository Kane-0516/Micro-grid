...
def _open_db_connection():
    try:
        ...
        print(f"[db] PostgreSQL unavailable for runtime product admin; using products.yaml fallback: {exc}")
        return None
...
def _get_yaml_setting_or_404(key: str) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Setting '{key}' is not supported.")
    if value is None:
...
def get_product_setting(key: str) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"[db] Failed to read runtime product setting '{key}'; using products.yaml fallback: {exc}")
        return _get_yaml_setting_or_404(key)
    finally:
...
def update_product_setting(key: str, payload: ProductAdminSettingPayload) -> dict[str, bool]:
    conn = _open_db_connection()
    try:
        if conn is None:
        ...
        _close_db_connection(conn)
        print(f"[db] Failed to update runtime product setting '{key}'; writing products.yaml fallback: {exc}")
        try:
            yaml_store.upsert_setting(key, payload.value)
        except KeyError:
            raise HTTPException(status_code=404, detail=f"Setting '{key}' is not supported.")
...
def create_product_item(category: ProductAdminCategory, payload: ProductAdminItemPayload) -> dict[str, bool]:
    conn = _open_db_connection()
    try:
        if conn is None:
...
def get_product_item(category: ProductAdminCategory, key: str) -> dict[str, Any]:
    conn = _open_db_connection()
    if conn is None:
        try:
            ...
            raise HTTPException(status_code=404, detail=f"Category '{category}' not found.")
        if item is None:
            raise HTTPException(status_code=404, detail=f"Item '{key}' not found in '{category}'.")
...
def update_product_item(category: ProductAdminCategory, key: str, payload: dict[str, Any]) -> dict[str, bool]:
    conn = _open_db_connection()
    try:
        if conn is None:
...
def delete_product_item(category: ProductAdminCategory, key: str) -> dict[str, bool]:
    conn = _open_db_connection()
    try:
        if conn is None:

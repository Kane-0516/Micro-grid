...
GEOCODE_ERROR_RESPONSES = {
    404: {"description": "No geocoding result found."},
    500: {"description": "Geocoder configuration is invalid."},
    502: {"description": "Unsupported geocoder response format."},
    503: {"description": "Geocoder provider is unavailable."},
}
...
class LocalPlace:
    """A known place in the local geocoding fallback table."""
...
LOCAL_CN_PLACES: tuple[LocalPlace, ...] = (
    ...
    LocalPlace(
        "中国", "西藏", "拉萨", 29.6520, 91.1721, ("西藏拉萨", "拉萨市", "拉萨")
    ),
)
...
PELIAS_PATH_MARKERS = ("/v1/search", "/v1/reverse")
...
def _place_match_score(normalized: str, place: LocalPlace) -> int:
...
def _local_cn_geocode(query: str) -> list[dict[str, Any]]:
    ...
    for place in LOCAL_CN_PLACES:
        score = _place_match_score(normalized, place)
        if score:
...
def _provider_unavailable_detail(url: str) -> str:
    if _is_pelias_url(url):
        return (
            "Pelias geocoder is unavailable. Start the Pelias API "
            "service and make sure its data import completed."
        )
    if "nominatim.openstreetmap.org" in url:
        return (
            "External geocoder is unreachable from this machine. "
            "Address search requires internet access, or you need to configure "
            "a local geocoder service."
        )
...
def _resolve_region(country_code: str | None) -> str:
    normalized = (
        (country_code or GEOCODER_DEFAULT_REGION or "cn").strip().lower()
    )
...
def _is_pelias_url(url: str) -> bool:
    ...
    return (
        any(marker in normalized for marker in PELIAS_PATH_MARKERS)
        or "pelias" in normalized
    )
...
def _append_state_code_candidates(cleaned: str, candidates: list[str]) -> None:
    state_pattern = re.compile(
        r"\b("
        + "|".join(re.escape(code) for code in US_STATE_ALIASES.values())
        + r")\b",
        re.IGNORECASE,
    )
    ...
    words = cleaned[: match.start()].strip().split()
    if not words:
        return
    candidates.append(f"{' '.join(words[-3:])} {state}")
    if len(words) >= 2:
        candidates.append(f"{' '.join(words[-2:])} {state}")


def _append_state_name_candidates(cleaned: str, candidates: list[str]) -> None:
    lowered = cleaned.lower()
    for state_name, state_code in US_STATE_ALIASES.items():
        index = lowered.rfind(state_name)
        if index < 0:
            continue
        words = cleaned[:index].strip().split()
        if words:
            city = " ".join(words[-3:])
            candidates.extend((f"{city} {state_code}", f"{city} {state_name}"))
        return


def _unique_queries(candidates: list[str], limit: int) -> list[str]:
    unique: list[str] = []
    seen: set[str] = set()
...
def _us_geocode_fallback_queries(query: str) -> list[str]:
    ...
    cleaned = re.sub(
        r"\b(usa|united states|united states of america)\b",
        " ",
        query,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"\b(suite|ste|unit|apt|apartment|floor|fl)\s+[-A-Z0-9]+\b",
        " ",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    candidates: list[str] = []
    _append_state_code_candidates(cleaned, candidates)
    _append_state_name_candidates(cleaned, candidates)
    ...
    return _unique_queries(candidates, 4)


def _pelias_feature_to_result(feature: Any) -> dict[str, Any] | None:
    ...
    display_name = _pick_first_text(
        [props.get("label"), props.get("name"), props.get("display_name")]
    )
    formatted_address = _pick_first_text(
        [props.get("label"), props.get("name")]
    )
    address = {
...
def _normalize_forward_payload(payload: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(payload, dict) and isinstance(payload.get("features"), list):
        results = (
            _pelias_feature_to_result(feature)
...
def _join_address_parts(parts: list[Any]) -> str:
    return ", ".join(
        str(part).strip()
        for part in parts
...
def _normalize_nominatim_reverse(payload: dict[str, Any]) -> dict[str, Any]:
    address = (
        payload.get("address")
        if isinstance(payload.get("address"), dict)
        else {}
    )
    locality = _pick_first_text(
        [
            address.get("city"),
            address.get("town"),
            address.get("village"),
    ...
    district = _pick_first_text(
        [address.get("county"), address.get("district"), address.get("suburb")]
    )
    formatted = _join_address_parts(
        [
            address.get("country"),
            address.get("state"),
            locality,
    ...
    return {
        "display_name": display_name,
        "formatted_address": formatted or display_name,
        "address": address,
        "raw": payload,
    }


def _normalize_pelias_reverse(payload: dict[str, Any]) -> dict[str, Any]:
    features = payload["features"]
    if not features:
        raise HTTPException(
            status_code=404, detail="No reverse geocoding result"
        )
    first = features[0] if isinstance(features[0], dict) else {}
    props = (
        first.get("properties")
        if isinstance(first.get("properties"), dict)
        else {}
    )
    formatted = _pick_first_text([props.get("label"), props.get("name")])
    joined = _join_address_parts(
        [
            props.get("country"),
            props.get("state"),
            props.get("city"),
...
def _normalize_reverse_payload(payload: Any) -> dict[str, Any]:
    if isinstance(payload, dict) and isinstance(
        ...
        return _normalize_nominatim_reverse(payload)
    if isinstance(payload, dict) and isinstance(payload.get("features"), list):
        return _normalize_pelias_reverse(payload)

    raise HTTPException(
        status_code=502, detail="Unsupported reverse geocoder response"
    )


@router.get("/geocode", responses=GEOCODE_ERROR_RESPONSES)
async def geocode(
    request: Request,
    q: str = Query(...),
    country_code: str | None = Query(default=None),
):
    """Forward-geocode a place name to coordinates via the Pelias proxy."""
    ...
    if not _is_pelias_url(geocoder_api_url):
        raise HTTPException(
            status_code=500, detail="Only Pelias geocoder URLs are supported."
        )
...
@router.get("/reverse-geocode", responses=GEOCODE_ERROR_RESPONSES)
async def reverse_geocode(
    ...
    """Reverse-geocode coordinates to a place name via the Pelias proxy."""

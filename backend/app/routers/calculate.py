...
def _calculate_cache_key(req: CalculateRequest, simulate: bool, simulation_defaults: dict | None = None) -> str:
    payload = {
        "simulate": simulate,
...
def _set_cached_calculate_response(cache_key: str, payload: dict) -> None:
    ...
    with _calculate_cache_lock:
        expired_keys = [key for key, (ttl, _) in _calculate_cache.items() if ttl <= now]
        for key in expired_keys:
            _calculate_cache.pop(key, None)
        if len(_calculate_cache) >= _CALCULATE_CACHE_MAX:
            oldest_key = min(_calculate_cache.items(), key=lambda item: item[1][0])[0]
...
def calculate(req: CalculateRequest, simulate: bool = False):
    ...
    try:
        ...
        from app.services.homer_economic_model import HomerEconomicSettings, generate_homer_economic_report
        from app.services.template_cost_engine import build_template_cost_breakdown
        ...
        baseline_diesel_eff = getattr(baseline_dg, "fuel_efficiency_kwh_per_liter", 3.5)
        actual_dg = catalog.diesel_generator(power_kw=actual_diesel_kw) if actual_diesel_kw > 0 else None
        actual_diesel_eff = (
            getattr(actual_dg, "fuel_efficiency_kwh_per_liter", baseline_diesel_eff)
        ...
        predictive_dispatch = (
            ...
            or "prediction" in {str(item).lower() for item in (req.emsAddons or [])}
        )
        simulation_dispatch_mode = "lp" if predictive_dispatch else req.dieselDispatchMode
        ...
        if simulate and pv_kw > 0:
            sim_r = run_pypsa(
                ...
                diesel_fuel_intercept_coeff=float(getattr(actual_dg, "fuel_intercept_coeff", 0.033)),
                diesel_fuel_slope_coeff=float(getattr(actual_dg, "fuel_slope_coeff", 0.273)),
            )
        else:
            sim_r = quick_estimate(
                pv_kw,
                actual_diesel_kw,
                compare_diesel_kw,
                battery_kwh,
        ...
        diesel_display_name = (
            actual_dg.display_name if actual_dg is not None
            else (f"{actual_diesel_kw:.0f}kW Diesel Generator" if actual_diesel_kw > 0 else "")
        )
        ...
        cost_breakdown = build_template_cost_breakdown(
            system_config={
                ...
                "dieselModel": (
                    actual_dg.model if actual_dg is not None and hasattr(actual_dg, "model")
                    else (f"DG-{actual_diesel_kw:.0f}kW" if actual_diesel_kw > 0 else "")
                ),
        ...
        proj = ProjectParameters(
            ...
            project_name             = (
                f"{actual_diesel_kw:.0f}kW Diesel / {pv_kw:.1f}kW PV Off-Grid Microgrid"
        ...
        report = generate_homer_economic_report(
            ...
            microgrid_generator_replacement_cost=float(actual_dg.price_usd) if actual_dg is not None else 0.0,
        ...
        missing_columns = [name for name in comparison_columns if name not in comp_df.columns]
        if missing_columns:
            raise KeyError(
                f"Comparison table columns mismatch. missing={missing_columns} available={list(comp_df.columns)}"
            )

        comparison_table = [
            {
                "year":              int(row["year"]),
                "mgAnnualCost":      float(row["mgAnnualCost"]),
                "dieselAnnualCost":  float(row["dieselAnnualCost"]),
                "mgCumulative":      float(row["mgCumulative"]),
                "dieselCumulative":  float(row["dieselCumulative"]),
                "mgLcoe":            float(row["mgLcoe"]),
                "dieselLcoe":        float(row["dieselLcoe"]),
                "annualRevenue":     float(row["annualRevenue"]),
                "cumulativeRevenue": float(row["cumulativeRevenue"]),
            }
        ...
        response_payload = {
            ...
            "systemConfig": {
                ...
                "dieselDispatchMode": sim_r.get("diesel_dispatch_mode", req.dieselDispatchMode),
            ...
            "summary": {
                ...
                "annualOperatingSavingsUsd": summary["annual_operating_savings_usd"],
                ...
                "microgridAnnualizedCostUsd": summary["microgrid_annualized_cost_usd"],
                "dieselOnlyAnnualizedCostUsd": summary["diesel_only_annualized_cost_usd"],
                "microgridOperatingCostUsd": summary["microgrid_operating_cost_usd"],
                "dieselOnlyOperatingCostUsd": summary["diesel_only_operating_cost_usd"],
                "microgridFixedOmUsd":   summary["microgrid_fixed_om_usd"],
                "microgridGeneratorMaintenanceUsd": summary["microgrid_generator_maintenance_usd"],
                "dieselOnlyGeneratorMaintenanceUsd": summary["diesel_only_generator_maintenance_usd"],
                "microgridCapitalNpcUsd": summary["microgrid_capital_npc_usd"],
                "microgridReplacementNpcUsd": summary["microgrid_replacement_npc_usd"],
                "microgridSalvageNpcUsd": summary["microgrid_salvage_npc_usd"],
                "dieselOnlyCapitalNpcUsd": summary["diesel_only_capital_npc_usd"],
                "dieselOnlyReplacementNpcUsd": summary["diesel_only_replacement_npc_usd"],
                "dieselOnlySalvageNpcUsd": summary["diesel_only_salvage_npc_usd"],
                ...
                "microgridGeneratorLifeYears": summary["microgrid_generator_life_years"],
                "dieselOnlyGeneratorLifeYears": summary["diesel_only_generator_life_years"],

"""
routers/layout.py — POST /api/layout/optimize
...
class LayoutOptimizeRequest(BaseModel):
    ...
    spacingM: Optional[float] = Field(None, description="相邻支架间距（m），默认从产品目录读取")
    bracketLengthM: Optional[float] = Field(None, description="支架长度（m），默认从产品目录读取")
    bracketWidthM: Optional[float] = Field(None, description="支架宽度（m），默认从产品目录读取")
...
def layout_optimize(req: LayoutOptimizeRequest):
    ...
    try:
        ...
        length = req.bracketLengthM if req.bracketLengthM is not None else cat_length
        width = req.bracketWidthM if req.bracketWidthM is not None else cat_width

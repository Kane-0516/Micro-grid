...
class CalculateRequest(BaseModel):
    """Input parameters for POST /api/calculate."""
    ...
    dieselDispatchMode: str = Field(
        "cc", description="lf | cc | cd | lp | proxy | uc"
    )
...
class SystemConfigResult(BaseModel):
    """Sized system configuration echoed back to the frontend."""
...
class CapexResult(BaseModel):
    """Capital expenditure breakdown, by cost category."""
...
class SimulationResult(BaseModel):
    """PyPSA simulation outputs: solar fraction, diesel usage, losses."""
...
class SummaryResult(BaseModel):
    """Economic summary: LCOE, NPC, payback, and cost breakdowns."""
    ...
    lcoeCrossoverYear: Optional[int] = None
    finalMgLcoe: float
...
class ComparisonRow(BaseModel):
    """One year of the microgrid-vs-diesel cost/revenue comparison table."""
...
class CalculateResponse(BaseModel):
    """Response envelope for POST /api/calculate."""

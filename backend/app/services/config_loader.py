...
_FUEL_EFFICIENCY = "fuel_efficiency_kwh_per_liter"
_FUEL_INTERCEPT = "fuel_intercept_coeff"
_FUEL_SLOPE = "fuel_slope_coeff"
_DEFAULT_TAG = " ← 默认"
...
class PVPanel:
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"价格: ${self.price_usd_per_wp:.3f}/Wp = "
            f"${self.price_usd_per_kw:.0f}/kW"
        )
...
class BracketSystem:
    ...
    footprint_length_m: float = 28.0  # 支架物理长度（m）
    footprint_width_m: float = 5.6  # 支架物理宽度（m）
    description: str = ""
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"{self.footprint_length_m}m × {self.footprint_width_m}m"
        )
...
class BatteryPack:
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"${self.price_usd:,.0f}/包 "
            f"(${self.price_usd_per_kwh:.0f}/kWh)  |  "
...
class Inverter:
    ...
    packs_per_inverter: int = (
        6  # 每台逆变器管理的电池包数（用于逆变器台数估算）
    )
    description: str = ""
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{self.model}  {self.display_name}\n"
            f"  {self.power_kw} kW  |  ${self.price_usd:,.0f}/台  |  "
            f"适配: {vlvl}  |  "
...
class DieselGenerator:
    ...
    fuel_intercept_coeff: float = 0.033
    fuel_slope_coeff: float = 0.273
    description: str = ""
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"燃油曲线 F0={self.fuel_intercept_coeff}, "
            f"F1={self.fuel_slope_coeff}"
        )
...
_BUILTIN_DEFAULTS = {
    "pv_panels": {
        ...
        "models": {
            "710W": {
                "display_name": "710Wp Ultra-High Eff. TOPCon N-type",
                "watts": 710,
                "price_usd_per_wp": 0.36,
                "efficiency_pct": 22.8,
            },
            "655W": {
                "display_name": "655Wp 高效单晶硅 PERC",
                "watts": 655,
                "price_usd_per_wp": 0.32,
                "efficiency_pct": 21.3,
            },
            "600W": {
                "display_name": "600Wp 单晶硅标准型",
                "watts": 600,
                "price_usd_per_wp": 0.30,
                "efficiency_pct": 20.5,
            },
            "550W": {
                "display_name": "550Wp 单晶硅",
                "watts": 550,
                "price_usd_per_wp": 0.30,
                "efficiency_pct": 20.0,
            },
            "400W": {
                "display_name": "400Wp 多晶硅经济型",
                "watts": 400,
                "price_usd_per_wp": 0.28,
                "efficiency_pct": 16.8,
            },
            "430W": {
                "display_name": "430Wp 单晶硅经济型",
                "watts": 430,
                "price_usd_per_wp": 0.29,
                "efficiency_pct": 18.5,
            },
        },
    },
    "bracket_systems": {
        ...
        "models": {
            "standard_32": {
                "display_name": "标准折叠支架（32块）",
                "panels_per_set": 32,
                "area_m2": 156.8,
                "footprint_length_m": 28.0,
                "footprint_width_m": 5.6,
            },
            "large_48": {
                "display_name": "大型折叠支架（48块）",
                "panels_per_set": 48,
                "area_m2": 235.2,
                "footprint_length_m": 42.0,
                "footprint_width_m": 5.6,
            },
            "compact_24": {
                "display_name": "紧凑折叠支架（24块）",
                "panels_per_set": 24,
                "area_m2": 117.6,
                "footprint_length_m": 21.0,
                "footprint_width_m": 5.6,
            },
        },
    },
    ...
    "inverters": {
        "models": {
            "INV-5000W-240V": {
                "display_name": "5kW 分相逆变器",
                "power_kw": 5.0,
                "price_usd": 7500,
                "voltage_levels": ["120V/240V"],
                "packs_per_inverter": 6,
            },
            "INV-7500W-208V": {
                "display_name": "7.5kW 三相逆变器",
                "power_kw": 7.5,
                "price_usd": 7500,
                "voltage_levels": ["120V/208V"],
                "packs_per_inverter": 6,
            },
            "INV-15000W-480V": {
                "display_name": "15kW 工业逆变器",
                "power_kw": 15.0,
                "price_usd": 15000,
                "voltage_levels": ["220V/380V", "230V/400V", "277V/480V"],
                "packs_per_inverter": 8,
            },
        },
        "voltage_default_map": {
            "120V/240V": "INV-5000W-240V",
            "120V/208V": "INV-7500W-208V",
            "220V/380V": "INV-15000W-480V",
            "230V/400V": "INV-15000W-480V",
    ...
    "diesel_generators": {
        ...
        "models": {
            "DG-20kW": {
                "display_name": "20kW 柴油机",
                "power_kw": 20,
                "price_usd": 22500,
                "fuel_efficiency_kwh_per_liter": 3.5,
            },
            "DG-30kW": {
                "display_name": "30kW 柴油机",
                "power_kw": 30,
                "price_usd": 33500,
                "fuel_efficiency_kwh_per_liter": 3.5,
                "fuel_intercept_coeff": 0.033,
                "fuel_slope_coeff": 0.224,
            },
            "DG-40kW": {
                "display_name": "40kW 柴油机",
                "power_kw": 40,
                "price_usd": 45000,
                "fuel_efficiency_kwh_per_liter": 3.5,
            },
            "DG-60kW": {
                "display_name": "60kW 柴油机",
                "power_kw": 60,
                "price_usd": 67500,
                "fuel_efficiency_kwh_per_liter": 3.6,
            },
            "DG-100kW": {
                "display_name": "100kW 柴油机",
                "power_kw": 100,
                "price_usd": 112500,
                "fuel_efficiency_kwh_per_liter": 3.8,
            },
        },
    },
    "accessories": {
        ...
        "pv_mounting_cost_per_set_usd": 19050.0,
        "intl_transport": {
            "base_usd": 3000.0,
    ...
    "simulation_defaults": {
        ...
        "default_year": 2020,
        "default_load_type": "residential",
        "diesel_dispatch_mode": "cc",
        "pv_generation_correction_factor": 1.10,
        "converter_kw_per_pv_kw": 0.50,
        "cycle_charging_target_load_pu": 0.58,
        "cycle_charging_start_soc_pu": 0.42,
    },
    "economic_defaults": {
        "diesel_price_usd_per_liter": 1.0,
        "electricity_price_usd_per_kwh": 0.35,
        "project_years": 25,
        "nominal_discount_rate_pct": 10.0,
...
class ProductCatalog:
    ...
    用法::
        ...
        cat = get_catalog()
        panel = cat.panel()  # 使用默认型号
        panel_655 = cat.panel("655W")  # 指定型号
        cat.list_all()  # 打印完整产品目录
    """

    def __init__(self, data: dict, source: str = "built-in"):
        """Load a product catalog from parsed data.

        Args:
            data: Parsed products.yaml content (or built-in defaults).
    ...
    @property
    def source(self) -> str:
        """数据来源描述（如 'products.yaml' 或 'built-in'）."""
        return self._source
    ...
    def panel(self, model: Optional[str] = None) -> PVPanel:
        ...
        model 为 None 时使用 default_model（yaml 中配置的默认型号）.
        """
        ...
        return PVPanel(
            model=name,
            display_name=d.get(_DISPLAY_NAME, name),
            watts=float(d["watts"]),
    ...
    def list_panels(self) -> None:
        ...
        for name, d in sec["models"].items():
            tag = _DEFAULT_TAG if name == default else ""
            wp = float(d["watts"])
            ppw = float(d["price_usd_per_wp"])
            eff = float(d.get("efficiency_pct", 0))
            print(
                f"  {name:<12} {d.get(_DISPLAY_NAME, ''):<26} "
                f"{wp:>5.0f}  ${ppw:>5.3f}  "
                f"${ppw * 1000:>5.0f}  {eff:>5.1f}%{tag}"
            )
        print()
    ...
    def bracket(self, model: Optional[str] = None) -> BracketSystem:
        ...
        return BracketSystem(
            ...
            footprint_length_m=float(d.get("footprint_length_m", 28.0)),
            footprint_width_m=float(d.get("footprint_width_m", 5.6)),
            description=str(d.get("description", "")),
        )
    ...
    def battery_pack(self, model: Optional[str] = None) -> BatteryPack:
        ...
        if d is None:
            raise KeyError(
                f"找不到电池型号 '{name}'。可用: {list(sec['models'].keys())}"
            )
        return BatteryPack(
            model=name,
            display_name=d.get(_DISPLAY_NAME, name),
    ...
    def battery_price_per_kwh(self, model: Optional[str] = None) -> float:
        ...
        try:
            ...
            return float(
                self._data["battery_packs"].get(
                    "price_usd_per_kwh_fallback", 310.0
                )
            )
    ...
    def inverter(self, model: str) -> Inverter:
        ...
        return Inverter(
            model=model,
            display_name=d.get(_DISPLAY_NAME, model),
            power_kw=float(d["power_kw"]),
            price_usd=float(d["price_usd"]),
    ...
    @staticmethod
    def _diesel_generator_from_data(model: str, data: dict) -> DieselGenerator:
        return DieselGenerator(
            model=model,
            display_name=data.get(_DISPLAY_NAME, model),
            power_kw=float(data["power_kw"]),
            price_usd=float(data["price_usd"]),
            fuel_efficiency_kwh_per_liter=float(
                data.get(_FUEL_EFFICIENCY, 3.5)
            ),
            fuel_intercept_coeff=float(data.get(_FUEL_INTERCEPT, 0.033)),
            fuel_slope_coeff=float(data.get(_FUEL_SLOPE, 0.273)),
        )
    ...
    def diesel_generator(
        ...
        优先按 model 查，其次按 power_kw 匹配最接近的型号；
        找不到时按通用 $/kW 单价估算.
        """
        ...
        if model and model in sec["models"]:
            return self._diesel_generator_from_data(model, sec["models"][model])
        ...
        if power_kw is not None:
            ...
            if best and best_diff <= power_kw * 0.25:  # 误差 ≤ 25%，认为匹配
                m_name, d = best
                return self._diesel_generator_from_data(m_name, d)
            # 找不到匹配型号 → 用通用单价估算
            ...
            return DieselGenerator(
                ...
                fuel_efficiency_kwh_per_liter=3.5,
                fuel_intercept_coeff=0.033,
                fuel_slope_coeff=0.273,
            )

        raise ValueError(
            "diesel_generator() 需要提供 model 或 power_kw 参数之一。"
        )
    ...
    def accessories(self) -> dict:
        """返回附属成本字典（固定项）."""
        return dict(
            self._data.get("accessories", _BUILTIN_DEFAULTS["accessories"])
        )
    ...
    def bracket_spacing_m(self) -> float:
        """返回相邻支架最小间距（m），从 bracket_systems.spacing_m 读取."""
        return float(
            self._data.get("bracket_systems", {}).get(
                "spacing_m", _BUILTIN_DEFAULTS["bracket_systems"]["spacing_m"]
            )
        )
    ...
    def site_layout(self) -> dict:
        """返回场地布局参数字典（tray 尺寸、柴发预留面积等）."""
        return dict(
            self._data.get("site_layout", _BUILTIN_DEFAULTS["site_layout"])
        )
    ...
    def simulation_defaults(self) -> dict:
        ...
        return dict(
            self._data.get(
                "simulation_defaults", _BUILTIN_DEFAULTS["simulation_defaults"]
            )
        )
    ...
    def list_all(self) -> None:
        ...
        for name, d in sec["models"].items():
            tag = _DEFAULT_TAG if name == default else ""
            print(
                f"  {name}  {d.get(_DISPLAY_NAME, '')}  "
                f"{d['panels_per_set']}pcs/set  {d['area_m2']}m2{tag}"
            )
        ...
        for name, d in sec["models"].items():
            ...
            prc = d["price_usd"]
            print(
                f"  {name:<14}  {d.get(_DISPLAY_NAME, '')}  {kwh}kWh  "
                f"${prc:,}  (${prc / kwh:.0f}/kWh){tag}"
            )
        ...
        for name, d in sec["models"].items():
            print(
                f"  {name:<12}  {d['power_kw']}kW  ${d['price_usd']:,}  "
                f"{d.get(_FUEL_EFFICIENCY, '?')}kWh/L"
            )
        print()
...
def load_catalog(path: Optional[str] = None) -> ProductCatalog:
    ...
    Args:
        path: YAML 文件路径；None 时自动查找同目录下的 products.yaml。
            若文件不存在或未安装 pyyaml，自动降级到内置默认值。

    Returns:
        加载得到的 ProductCatalog。
    """
    ...
    if os.path.exists(yaml_path) and _YAML_AVAILABLE:
        try:
            ...
            print(
                f"  [config_loader] 警告：读取 {yaml_path} 失败（{e}），"
                "使用内置默认值。"
            )

    if not _YAML_AVAILABLE:
        print(
            "  [config_loader] 提示：未安装 pyyaml，使用内置默认值。"
            "（pip install pyyaml 可启用 YAML 配置）"
        )
...
def get_catalog(path: Optional[str] = None) -> ProductCatalog:
    ...
    resolved_path = os.path.abspath(path) if path else None
    if _catalog is None or (
        resolved_path is not None and resolved_path != _catalog_path
    ):
        _catalog = load_catalog(resolved_path)
        _catalog_path = resolved_path
    return _catalog

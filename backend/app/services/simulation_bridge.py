...
from app.services.microgrid_designer import (
    MicrogridDesigner,
)
...
class DieselFuelModel:
    ...
    比 MicrogridDesigner 中使用的固定值（3.2 kWh/L）更精确：对不同负载率给出
    不同效率，特别适合纯柴油模式（低负载率场景）。

    模型：eff(f) = rated_eff × (A + B × f)，其中 f = P_output / P_rated
    （负载率）。默认 A=0.70, B=0.30，满负载时 eff = rated_eff。

    案例验证（40kW 柴油机，rated_eff=3.5 kWh/L）：纯柴油模式
    avg f = 15kW/40kW = 0.375，eff = 3.5 × (0.70 + 0.30×0.375) = 2.844 kWh/L，
    ...
    def __init__(
        ...
        """Configure the part-load fuel efficiency curve.

        Args:
            rated_power_kw: Generator rated power, in kW.
            rated_efficiency_kwh_per_liter: Efficiency at full load.
            A: Efficiency curve intercept coefficient (A + B must be 1.0).
            B: Efficiency curve slope coefficient (A + B must be 1.0).
        """
        assert abs(A + B - 1.0) < 1e-6, (
            "A + B 必须等于 1.0（满负载效率即额定效率）"
        )
    ...
    def effective_kwh_per_liter(
        self, annual_load_kwh: float, hours: float = 8760.0
    ) -> float:
    ...
    def print_curve(self) -> None:
        """打印效率曲线（辅助调试）."""
        print(
            f"\n  柴油机效率曲线（额定 {self.P_rated} kW，"
            f"{self.eff_rated} kWh/L @ 满载）:"
        )
        print(
            f"  {'负载率':>8}  {'功率kW':>8}  "
            f"{'效率kWh/L':>12}  {'油耗L/h':>10}"
        )
        for pct in [30, 50, 75, 100]:
            P = self.P_rated * pct / 100
            print(
                f"  {pct:>7}%  {P:>8.1f}  {self.efficiency(P):>12.3f}  "
                f"{self.hourly_liters(P):>10.3f}"
            )
...
class SystemAutoSizer:
    ...
    def __init__(
        ...
        """Configure the diesel-driven sizing heuristic.

        Args:
            diesel_kw: Diesel generator capacity, in kW.
            pv_ratio: PV-to-diesel capacity ratio.
            bat_ratio: Battery (kWh)-to-diesel (kW) ratio.
        """
...
def run_from_minimal_inputs(
    ...
    current_schedule: Optional[
        list
    ] = None,  # 电流时刻表 [(h_start,h_end,amps),...]
    annual_kwh_override: Optional[
        float
    ] = None,  # 归一化年用电量（配合电流规格使用）
    # ── 必填：柴油机规格 ──────────────────────────────────────
    ...
    # 使用部分负载效率模型（比固定值更准确）
    use_partload_fuel_model: bool = True,
    # ── 可选：系统成本（None=按单价估算）─────────────────────
    ...
    Args:
        ...
        current_schedule: 电流时刻表，例如
            [(8,10,140),(10,17,50),(17,20,140),(20,32,20)].
        annual_kwh_override: 将电流时序归一化到此年用电量
            （与 current_schedule 配合）.
        ...
        use_partload_fuel_model: True 使用部分负载效率模型，
            False 使用固定 3.2 kWh/L.
        capex: 可选 CAPEX，不填则按系统规格自动估算.
        mg_om: 可选微电网运维参数，不填则自动估算.
        diesel_om: 可选柴油机运维参数，不填则自动估算.
        verbose: 是否打印进度与 PyPSA 求解器输出.

    Returns:
        MicrogridDesigner.design() 的完整返回值，额外包含：
    ...
    if verbose:
        ...
        print(f"\n{'=' * 62}")
        print("  微电网方案生成器  [simulation_bridge → MicrogridDesigner]")
        print(f"{'=' * 62}")
    ...
    if use_partload_fuel_model and annual_load_kwh:
        ...
        if verbose:
            print(
                f"  柴油效率模型 : 部分负载（等效 {effective_eff:.3f} kWh/L）"
            )
    else:
...
if __name__ == "__main__":
    ...
    print(
        f"    微电网年用油量    : {ps2['mg_diesel_liters']:>10,.0f} L  "
        "(HOMER Pro: 5,599 L)"
    )
    print(
        f"    纯柴油年用油量    : {ps2['diesel_only_liters']:>10,.0f} L  "
        "(HOMER Pro: 45,764 L)"
    )
    be2 = s2["breakeven_year"]
    payback_str = f"第 {be2} 年" if be2 else ">分析期"
    print(f"    投资回本年        : {payback_str}")
    ...
    print("  HOMER Pro 参考值            : 45,764 L")
    error_pct = abs(do_liters - 45764) / 45764 * 100
    print(f"  误差                        : {error_pct:.1f}%")

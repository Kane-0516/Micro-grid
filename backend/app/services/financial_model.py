...
def calculate_annual_savings(
    ...
    Args:
        sim_results: PyPSA 仿真结果
            （来自 OffGridMicrogridSimulator.run_simulation()）。
        config: 系统配置。
        econ: 经济性参数；不传则使用默认值。
        op_costs: 运营成本参数；不传则使用默认值。
    ...
    # ─── 年收益（节省的电费或柴油费）─────────────
    # 实际被光储系统满足的负荷
    served_by_solar_storage = (
        annual_load_kwh
        - sim_results.get("annual_load_shed_kwh", 0)
        - annual_diesel_kwh
    )
...
def _build_cashflows(
    effective_capex: float,
    year1_net_savings_cny: float,
    battery_cost_cny: float,
    econ: EconomicParameters,
    op_costs: OperationCosts,
) -> tuple[list[float], float | None]:
    cashflows = [-effective_capex]
    cumulative_cashflow = -effective_capex
    dynamic_payback_years = None

    for year in range(1, econ.analysis_years + 1):
        annual_savings = year1_net_savings_cny * (
            1 + econ.annual_electricity_growth
        ) ** (year - 1)
        annual_savings *= (1 - econ.pv_degradation_rate) ** year
        replacement_due = (
            op_costs.battery_replacement_years > 0
            and year % op_costs.battery_replacement_years == 0
        )
        replacement_cost = (
            battery_cost_cny * op_costs.battery_replacement_cost_rate
            if replacement_due
            else 0
        )
        net_cashflow = annual_savings - replacement_cost
        cashflows.append(net_cashflow)

        previous_cashflow = cumulative_cashflow
        cumulative_cashflow += net_cashflow
        if dynamic_payback_years is None and cumulative_cashflow >= 0:
            dynamic_payback_years = year - 1 - previous_cashflow / net_cashflow

    return cashflows, dynamic_payback_years
...
def calculate_payback(
    ...
    Args:
        total_capex_cny: 总初始投资（元）。
        year1_net_savings_cny: 第一年净节省（元）。
        capex_details: 投资明细（来自 calculate_capex）。
        econ: 经济性参数；不传则使用默认值。
        op_costs: 运营参数；不传则使用默认值。
        battery_cost_cny: 电池初始成本（用于计算更换费用）。
    ...
    cashflows, dynamic_payback_years = _build_cashflows(
        effective_capex,
        year1_net_savings_cny,
        battery_cost_cny,
        econ,
        op_costs,
    )
    ...
    # ── NPV 计算 ────────────────────────────
    npv = sum(
        cf / (1 + econ.discount_rate) ** t for t, cf in enumerate(cashflows)
    )
    ...
    return {
        ...
        "dynamic_payback_years": round(dynamic_payback_years, 1)
        if dynamic_payback_years
        else None,
        "npv_cny": round(npv, 0),
...
def _calculate_irr(
    cashflows: list, max_iter: int = 1000, tol: float = 1e-6
) -> float | None:
    """用二分法计算内部收益率（IRR）."""
...
def format_report(
    ...
    diesel_backup = (
        "无"
        if config.diesel_capacity_kw == 0
        else f"{config.diesel_capacity_kw:.0f} kW"
    )
    irr_percent = payback["irr_percent"]
    irr_text = f"{irr_percent:.1f}%" if irr_percent else "N/A"

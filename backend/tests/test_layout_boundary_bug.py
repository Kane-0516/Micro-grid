...
Bug 描述：
  `_rect_fits` 仅通过 9 个离散采样点（4 角 + 4 边中点 + 1 中心）
  判断矩形是否在多边形内。当多边形边界从两个相邻采样点之间穿过时，
  矩形被错误地判定为"适合"。

测试策略：
  构造矩形/多边形对，使得所有 9 个采样点都在多边形内，
  但至少一条矩形边与多边形边相交。在未修复代码上，
  `_rect_fits` 会错误返回 True（测试断言 False，因此测试会 FAIL）。
  测试 FAIL 即确认 bug 存在。
...
def _segments_intersect_check(p1, p2, p3, p4):
    ...
    def on_segment(p, q, r):
        """检查点 q 是否在线段 pr 上."""
        return (
            min(p.x, r.x) <= q.x + 1e-9 <= max(p.x, r.x) + 1e-9
            and min(p.y, r.y) <= q.y + 1e-9 <= max(p.y, r.y) + 1e-9
        )

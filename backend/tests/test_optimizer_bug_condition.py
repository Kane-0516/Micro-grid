...
class TestNPCBasedRanking:
    ...
    def test_ranking_is_npc_based(self):
        ...
        is_sorted_by_score_desc = all(
            scores[i] >= scores[i + 1] - 1e-6 for i in range(len(scores) - 1)
        )

        actual = [
            (f"sets={o.bracket_sets}, npc={-o.score:.0f}") for o in results
        ]
        assert is_sorted_by_score_desc, f"结果未按 NPC 升序排列。实际: {actual}"

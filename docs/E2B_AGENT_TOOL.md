...
persistent server, no database. Uses **方式 2** from
`e2b-microgrid-guide.md`: the sandbox image only has the Python
dependencies pre-installed; the actual backend code is pulled fresh via
`git clone` from GitHub on every call. Updating the tool is then just
`git push` — no template rebuild needed.
